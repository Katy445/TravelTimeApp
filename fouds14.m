...
travmt=0;
slown=1.0/veln(iz,ix);
mf2=sqrt(5);  % distance between two diagonally connected points  
...
% !           each quadrant.
                ...
                if nsts(k,j)==0
               	swsol=1;
                if nsts(kk,jj)==0
                    u=mf2*dnx;				%!KT 17/5/17 diagonal nodes are sqrt(2)*dnx from active node (isotropic grid spacing only)
                    v=mf2*dnz;
                    a=2;
